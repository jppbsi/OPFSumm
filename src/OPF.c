...
/*----------------------------- UNSUPERVISED OPF -----------------------------*/
// Training function: it computes unsupervised training for pre-computed best k
...
void opf_OPFClustering(Subgraph *sg) {
    ...
    //comentar esse FOR no caso de sumarização
    for (i = 0; i < sg->nnodes; i++) {                                          // add arcs to guarantee symmetry on plateaus
        adj_i = sg->node[i].adj;

        while (adj_i != NULL) {
            j  = adj_i->elem;
            if (sg->node[i].dens == sg->node[j].dens) {
                adj_j = sg->node[j].adj;                                        // insert i in the adjacency of j if it is not there.
                insert_i = 1;
                ...
                if (insert_i) InsertSet(&(sg->node[j].adj), i);
            }
    ...
    for (p = 0; p < sg->nnodes; p++) {                                          // nodes initialization
        pathval[p] = sg->node[p].pathval;                                       // path value
        sg->node[p].pred = NIL;                                                 // without predecessor (NIL = -1)
        sg->node[p].root = p;                                                   // the root is the node itself
        InsertRealHeap(Q, p);                                                   // put the node on the heap
    }

    l = 0; i = 0;                                                               // 'l' = number of labels ; 'i' = number of nodes
    while (!IsEmptyRealHeap(Q)) {
        RemoveRealHeap(Q, &p);                                                  // the node with maximum path value is removed
        sg->ordered_list_of_nodes[i] = p;                                       // ordered by maximum value
        i++;

        if (sg->node[p].pred == NIL) {
            pathval[p] = sg->node[p].dens;                                      // path value becomes the node density value
            sg->node[p].label = l;                                              // assign a label to the node
            l++;                                                                // increases the number of labels
        }

        sg->node[p].pathval = pathval[p];

        for (Saux = sg->node[p].adj; Saux != NULL; Saux = Saux->next) {         // look for all adjacent nodes of 'p'
            q = Saux->elem;

            if ( Q->color[q] != BLACK ) {
                tmp = MIN(pathval[p], sg->node[q].dens);                        // connectivity function (original version)

                if (tmp > pathval[q]) {
                    UpdateRealHeap(Q, q, tmp);                                  // updates the position of node 'q' in the heap
                    sg->node[q].pred = p;                                       // node 'p' becomes the predecessor of node 'q'
                    sg->node[q].root = sg->node[p].root;                        // 'p's root is assigned to the node 'q' (belongs to the same cluster rooted by 'p')
                    sg->node[q].label = sg->node[p].label;                      // 'p's label is assigned to node 'q'
                }
            }
        }
    }

    sg->nlabels = l;                                                            // updates the number of labels
    DestroyRealHeap(&Q);                                                        // destroys the heap
    free(pathval);                                                              // liberate memory from 'pathval'
}
...
Subgraph *opf_ReadModelFile(char *file){
  ...
  if (fread(&nnodes, sizeof(int), 1, fp) != 1) Error("Could not read number of nodes","opf_ReadModelFile");

  g = CreateSubgraph(nnodes);
  if (fread(&g->nlabels, sizeof(int), 1, fp) != 1) Error("Could not read number of labels","opf_ReadModelFile");
  if(fread(&g->nfeats, sizeof(int), 1, fp) != 1) Error("Could not read number of features","opf_ReadModelFile");

  /* for supervised opf by pdf */
  if (fread(&g->df, sizeof(float), 1, fp) != 1) Error("Could not read adjacency radius","opf_ReadModelFile");
  if (fread(&g->bestk, sizeof(int), 1, fp) != 1) Error("Could not read the best k","opf_ReadModelFile");
  if (fread(&g->K, sizeof(float), 1, fp) != 1) Error("Could not read K","opf_ReadModelFile");
  if (fread(&g->mindens, sizeof(float), 1, fp) != 1) Error("Could not read minimum density","opf_ReadModelFile");
  if (fread(&g->maxdens, sizeof(float), 1, fp) != 1) Error("Could not read maximum density","opf_ReadModelFile");

  /* reading nodes' information */
  for (i = 0; i < g->nnodes; i++){
    g->node[i].feat = (float *)malloc(g->nfeats*sizeof(float));
    if (fread(&g->node[i].position, sizeof(int), 1, fp) != 1) Error("Could not read node position","opf_ReadModelFile");
    if (fread(&g->node[i].truelabel, sizeof(int), 1, fp) != 1) Error("Could not read node true label","opf_ReadModelFile");
    if (fread(&g->node[i].pred, sizeof(int), 1, fp) != 1) Error("Could not read node predecessor","opf_ReadModelFile");
    if (fread(&g->node[i].label, sizeof(int), 1, fp) != 1) Error("Could not read node label","opf_ReadModelFile");
    if (fread(&g->node[i].pathval, sizeof(float), 1, fp) != 1) Error("Could not read node path value","opf_ReadModelFile");
    if (fread(&g->node[i].radius, sizeof(float), 1, fp) != 1) Error("Could not read node adjacency radius","opf_ReadModelFile");
	if (fread(&g->node[i].dens, sizeof(float), 1, fp) != 1) Error("Could not read node density value","opf_ReadModelFile");

    for (j = 0; j < g->nfeats; j++)
      if (fread(&g->node[i].feat[j], sizeof(float), 1, fp) != 1) Error("Could not read node features","opf_ReadModelFile");
  }

  for (i = 0; i < g->nnodes; i++)
    if (fread(&g->ordered_list_of_nodes[i], sizeof(int), 1, fp) != 1) Error("Could not read ordered list of nodes","opf_ReadModelFile");
...
void opf_NormalizeFeatures(Subgraph *sg) {
    int i, j;
    float *mean = (float *)calloc(sg->nfeats, sizeof(float)),
           *std = (float *)calloc(sg->nfeats, sizeof(int));
...
void opf_PDF(Subgraph *sg) {
    ...
    for (i = 0; i < sg->nnodes; i++) {
        ...
        while (adj != NULL) {
            if (!opf_PrecomputedDistance) {
                UpdateFeatIndexes(i, adj->elem, sg);
        ...
        if (value[i] < sg->mindens) sg->mindens = value[i];
        if (value[i] > sg->maxdens) sg->maxdens = value[i];
    }
    ...
    if (sg->mindens == sg->maxdens) {
        for (i = 0; i < sg->nnodes; i++) {
            ...
            sg->node[i].pathval = opf_MAXDENS - 1.0;
        }
    } else {
        for (i = 0; i < sg->nnodes; i++) {
            sg->node[i].dens = ((float)(opf_MAXDENS - 1.0) * (value[i] - sg->mindens) / (float)(sg->maxdens - sg->mindens)) + 1.0;
            sg->node[i].pathval = sg->node[i].dens - 1.0;
        }
    }
    free(value);
}
...
float opf_CombinedDist(Subgraph *sg, int f1, int f2) {
    float featdist = opf_ArcWeight(sg->node[f1].feat, sg->node[f2].feat, sg->nfeats),
      posdist = fabs(sg->node[f1].normpos - sg->node[f2].normpos),
      dist = (featdist / sg->maxdist) + (1 - sg->alfa) * posdist;
      //dist = (1 - sg->alfa) * (featdist / sg->maxdist) + sg->alfa * posdist;

    //eucl = opf_EuclDistLog(sg->node[f1].feat, sg->node[f2].feat, sg->nfeats);
    //posdist = fabs(sg->node[f1].normpos - sg->node[f2].normpos);
    //dist = eucl / sg->maxdist + (1.0 - sg->alfa) * posdist; //* zncc;
    return dist;
}

float opf_TemporalDist(float *f1, float *f2, int n) {
    int i = opfsumm_TempVariables.fst_feat_index,
        j = opfsumm_TempVariables.snd_feat_index;
    float   spatial = opf_EuclDistLog(f1, f2, n),
            temporal = fabs(opfsumm_TempVariables.g->node[i].normpos - opfsumm_TempVariables.g->node[j].normpos),
            dist = (spatial / opfsumm_TempVariables.g->maxdist) + (opfsumm_TempVariables.alpha * temporal);
...
// Create adjacent list in subgraph: a knn graph.
// Returns an array with the maximum distances for each k = 1, 2, ... , kmax
...
float opf_NormalizedCutToKmax(Subgraph *sg) {
    ...
    for (p = 0; p < sg->nnodes; p++) {
        const int nadj = sg->node[p].nplatadj + kmax; //for plateaus the number of adjacent nodes will be greater than the current maximum, but they should be considered
