...
typedef struct _snode {
  ...
  int position;                                                                 // index in the feature space (frame id)
  float normpos;                                                                // normalized index (frame id)
  float *feat;                                                                  // feature vector
  char status;                                                                  // 0 - nothing, 1 - prototype
  char relevant;                                                                // 0 - irrelevant, 1 - relevant
  int nplatadj;                                                                 // holds the amount of adjacent nodes on plateaus it is used to optimize opf_BestkMinCut because a knn graph need only be built for kmax, but the opf_PDF and opf_NormalizedCut computation need only be done for the current k or until k + nplatadj is reached.
...
typedef struct _subgraph {
  ...
  float maxdist;                                                                // maximum distance value in the graph
  float alfa;                                                                   // weight control value for temporal distance
