...
#define opf_MAXDENS			100.0 //1000.0
...
float opf_dLogfunction(float n);
/*--------------------------- Distance functions ---------------------------- */
float opf_EuclDist(float *f1, float *f2, int n); //Computes Euclidean distance between feature vectors
float opf_EuclDistLog(float *f1, float *f2, int n); //Computes Euclidean distance between feature vectors and applies logarithmic function
...
float opf_ZnccDist(float *f1, float *f2, int n); // Computes ZNCC distance between feature vectors
float opf_CombinedDist(Subgraph *sg, int f1, int f2); // Computes a combined distance (euclidian + frame position) specifically for video summarization
float opf_dLogDist(float *f1, float *f2, int n); // Computes dLog distance between feature vectors

/* ------------ Auxiliary functions used to optimize BestkMinCut ------------ */
float* opf_CreateArcs2(Subgraph *sg, int kmax); //Creates arcs for each node (adjacency relation) and returns the maximum distances for each k=1,2,...,kmax
